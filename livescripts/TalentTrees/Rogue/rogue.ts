...
export function ReloadRogueTree() {
    ...
    SetTalentNode(Talent, TAB, 2, 1, 0, false, 2**(SpecTabs.ASSR - 1), CreateArray<uint32>([Talent]), EmptyPrereqs, EmptySpellArray, EmptySpellArray)
    Talent = GetID(`Spell`, `dh-spells`, `rog-gen-crimsonvial`)
    SetTalentNode(Talent, TAB, 5, 1, 0, false, 2**(SpecTabs.CORR - 1), CreateArray<uint32>([Talent]), EmptyPrereqs, EmptySpellArray, EmptySpellArray)
    Talent = GetID(`Spell`, `dh-spells`, `rog-gen-sap`)
    SetTalentNode(Talent, TAB, 8, 1, 0, false, 2**(SpecTabs.SUBR - 1), CreateArray<uint32>([Talent]), EmptyPrereqs, EmptySpellArray, EmptySpellArray)
