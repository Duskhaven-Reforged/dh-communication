...
export function ReloadRogueCorsairTree() {
    ...
    SetTalentNode(Talent, TAB, 2, 3, 0, false, 0, CreateArray<uint32>([Talent]), CreateDictionary<uint32, uint8>({[GetID(`Spell`, `dh-spells`, `rog-cor-neptulonslockes`)]: 1}), EmptySpellArray, EmptySpellArray)
    Talent = GetID(`Spell`, `dh-spells`, `rog-cor-curseddoubloon`)
    SetTalentNode(Talent, TAB, 5, 3, 0, true, 0, CreateArray<uint32>([Talent]), CreateDictionary<uint32, uint8>({[GetID(`Spell`, `dh-spells`, `rog-cor-neptulonslockes`)]: 1, [GetID(`Spell`, `dh-spells`, `rog-cor-flourishtheblade`)]: 1, [GetID(`Spell`, `dh-spells`, `rog-cor-spectralshot`)]: 1}), EmptySpellArray, EmptySpellArray)
