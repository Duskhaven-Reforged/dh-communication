...
export function ReloadShamanEnhTree() {
    ...
    SetTalentNode(Talent, TAB, 3, 3, 0, true, 0, CreateArray<uint32>([Talent]), CreateDictionary<uint32, uint8>({[GetID(`Spell`, `dh-spells`, `sha-wat-ferallunge`)]: 1}), EmptySpellArray, EmptySpellArray)
    Talent = GetID(`Spell`, `dh-spells`, `sha-enh-feralinstincts`)
    SetTalentNode(Talent, TAB, 5, 3, 0, true, 0, CreateArray<uint32>([Talent]), CreateDictionary<uint32, uint8>({[GetID(`Spell`, `dh-spells`, `sha-wat-ferallunge`)]: 1, [GetID(`Spell`, `dh-spells`, `sha-enh-improvedmaelstromweapon`)]: 1}), EmptySpellArray, EmptySpellArray)
