import { EmptyPrereqs, EmptySpellArray, SetTalentNode } from "../TalentTreeLoader"

export function ReloadArmsWarrTree() {
    let TAB = 1

    let Talent : uint32 = GetID(`Spell`, 'dh-spells', 'warr-arm-mortal-strike')
    SetTalentNode(Talent, TAB, 6, 1, 0, true, false, CreateArray<uint32>([Talent]), EmptyPrereqs, EmptySpellArray, EmptySpellArray)
    ...
    SetTalentNode(Talent, TAB, 6, 2, 0, true, false, CreateArray<uint32>([Talent]), CreateDictionary<uint32, uint8>({[GetID(`Spell`, `dh-spells`, `warr-arm-mortal-strike`)]: 1}), EmptySpellArray, EmptySpellArray)
    ...
    // TODO add choice node for OP mods
